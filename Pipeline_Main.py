...
train_data['Credit Score'] = [score/10 if score > 800 else score for score in train_data['Credit Score']]
test_data['Credit Score'] = [score/10 if score > 800 else score for score in test_data['Credit Score']]
...
feature_transform_dict = OrderedDict({'Years in current job': {
...
num_feature_impute_dict = OrderedDict({'Bankruptcies': {'strategy': 'based_on_other_column',
                                       ...
                                       'Years in current job': {'missing_values': np.nan,
                                                                'strategy': 'constant',
                                                                'constant_value': 0},
                                       'Annual Income': {'strategy': 'based_on_other_column',
                                            'other_column': ['Monthly_Debt_Binned'],
                                            'group_by_transformation': True},                                       
                                       'Credit Score': {'strategy': 'based_on_other_column',
                                            'other_column': ['Current_Loan_Amount_Binned'],
                                            'group_by_transformation': True},
                                       })


cat_lbl_encode_list = ['Home Ownership', 'Purpose', 'Term',]

cat_1hot_encode_list = ['Home Ownership', 'Purpose', 'Term',]
...
logger.info(f"\nFinal Tranformed Dataframe:\n{transformed_df.sample(100).to_string()}")
